...
async function run() {
  try {
    ...
    const classesCollection = client.db('sportsCamp').collection("classes")
    const cartCollection = client.db('sportsCamp').collection("carts")
    ...
    app.post('/feedback/:id', async (req, res) => {
      ...
      const result = await classesCollection.updateOne(filter, addFedback)
      res.send(result);
    })
    ...
    app.patch('/classNames/:id',verifyJWT, async (req, res) => {
      const id = req.params.id;
      const filter = { _id: new ObjectId(id) }
...
app.get('/', (req, res) => {
  res.send('Sports camp is open')
})


app.listen(port, () => {
  console.log(`Sports camp is open on prot ${port} `)
})
